...
parser.add_argument("--lmode", required=False, choices=["reconstruct", "relit"])
# export options
...
def create_generator(generator_inputs,L_coeff_in,lmode='reconstruct'):
    ...
    with tf.variable_scope("shader"):
        ...
        if(lmode == 'relit'):
            L_flat = tf.reshape(L_coeff_in, [-1, 27])
        else:
    ...
    model['I_bg'] = I_bg
    model['I_fg'] = I_fg
    model['L'] = tf.reshape(z_light,[-1,27])
    model['shading'] = S
    model['img_bg'] = I_bg
    model['img_fg'] = I_fg
    return model
...
def create_model( input_data):
    ...
    lcoeffs = tf.reshape(input_data.lcoeffs,[-1,27])

    if (a.lmode=='reconstruct'):
        with tf.variable_scope("generator"):
            gen_outputs1 = create_generator(inputs,lcoeffs,'reconstruct')
            outputs = gen_outputs['reconstructed']
    else:
        with tf.variable_scope("generator"):
            gen_outputs1 = create_generator(targets,lcoeffs,'reconstruct')
        with tf.variable_scope("generator",reuse=True):
            gen_outputs = create_generator(inputs,gen_outputs1['L'],'relit')
            outputs = gen_outputs['reconstructed']
    ...
    for i, in_path in enumerate(fetches["paths"]):
        ...
        for kind in ["inputs", "outputs", "targets"]:
            ...
            with open(out_path, "wb") as f:
                f.write(contents)
	lcoeff_str =""
        for lcoeff in fetches["lcoeffs"][i]:
		lcoeff_str = "%s%0.5f,"%(lcoeff_str,lcoeff)
	fileset["lcoeffs"]=lcoeff_str
...
def append_index(filesets, step=False, mode="train"):
    ...
    if os.path.exists(index_path):
        ...
        index.write("<th>name</th><th>input</th><th>output</th><th>target</th><th>Lcoeffs</th></tr>\n")
    ...
    for fileset in filesets:
        ...
        for kind in ["inputs", "outputs", "targets"]:
            index.write("<td><img src='%s/images/%s'></td>\n" % (mode, fileset[kind]))
        index.write("<td>'%s'</td>" % ( fileset["lcoeffs"]))
...
def main():
    ...
    with sv.managed_session() as sess:
        ...
        if a.mode == "test":
            # testing
            steps_per_epoch = sess.run(test_examples.steps_per_epoch)
