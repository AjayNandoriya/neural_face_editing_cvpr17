...
parser.add_argument("--mask_weight", type=float, default=100.0, help="weight on mask for generator")
parser.add_argument("--lcoeff_weight", type=float, default=100.0, help="weight on light coefficient for generator")
parser.add_argument("--normal_weight", type=float, default=100.0, help="weight on normal for generator")
parser.add_argument("--albedo_weight", type=float, default=100.0, help="weight on albedo for generator")
parser.add_argument("--shading_weight", type=float, default=100.0, help="weight on shading for generator")
...
def decoder_stack(input_layer):
    layers = [input_layer]
    layer_specs = [a.ngf * 8, a.ngf * 8, a.ngf * 8, a.ngf * 8, a.ngf * 8, a.ngf * 8, a.ngf * 4, a.ngf * 2, a.ngf]
...
def create_generator(generator_inputs):
    ...
    with tf.variable_scope("encoder_3"):
        conv3 = tf.layers.conv2d(inputs=layers[-1], filters=a.ngf * 4, kernel_size=3, padding='same',
                                 activation=tf.nn.relu)
        pool3 = tf.layers.max_pooling2d(inputs=conv3, pool_size=[2, 2], strides=2)
        layers.append(pool3)
    with tf.variable_scope("encoder_4"):
        conv4 = tf.layers.conv2d(inputs=layers[-1], filters=a.ngf * 8, kernel_size=3, padding='same',
                                 activation=tf.nn.relu)
        pool4 = tf.layers.max_pooling2d(inputs=conv4, pool_size=[2, 2], strides=2)
        layers.append(pool4)
    with tf.variable_scope("encoder_5"):
        conv5 = tf.layers.conv2d(inputs=layers[-1], filters=a.ngf * 8, kernel_size=3, padding='same',
                                 activation=tf.nn.relu)
    ...
    with tf.variable_scope("encoder_background"):
        z_bg = tf.layers.conv2d(inputs=layers[-1], filters=a.ngf * 8, kernel_size=1, padding='same',
                                activation=tf.nn.relu)
    with tf.variable_scope("encoder_mask"):
        z_mask = tf.layers.conv2d(inputs=layers[-1], filters=a.ngf * 8, kernel_size=1, padding='same',
                                  activation=tf.nn.relu)
    with tf.variable_scope("encoder_normal"):
        z_normal = tf.layers.conv2d(inputs=layers[-1], filters=a.ngf * 8, kernel_size=1, padding='same',
                                    activation=tf.nn.relu)
    ...
    with tf.variable_scope("encoder_albedo"):
        z_albedo = tf.layers.conv2d(inputs=layers[-1], filters=a.ngf * 8, kernel_size=1, padding='same',
                                    activation=tf.nn.relu)
